...
(function (F, x, Oa) {
    ...
    var L = x.noop, za = x.extend, I = x.element, q = x.forEach, Y = x.isArray, M = x.isString, oa = x.isObject, pa = x.isUndefined, qa = x.isDefined, Da = x.isFunction, ra = x.isElement, N, sa, W, ta;
    ...
    x.module("ngAnimate", []).provider("$$body", function () {
        ...
        ["$animateProvider", function (a) {
            ...
            this.$get = ["$$rAF", "$rootScope", "$rootElement", "$document", "$$body", "$$HashMap", "$$animation", "$$AnimateRunner", "$templateRequest", "$$jqLite", "$$forceReflow", function (d, y, v, z, l, s, O, x, t, la, E) {
                ...
                function u(a, g, m) {
                    ...
                    if (!C(k))return w(), e;
                    var l = 0 <= ["enter", "move", "leave"].indexOf(g), u = !H || U.get(B), k = !u && A.get(B) || {}, v = !!k.state;
                    ...
                    y.$$postDigest(function () {
                        var b = A.get(B), h = !b, b = b || {}, J = 0 < (a.parent() || []).length && ("animate" === b.event || b.structural || c(b.options));
                        if (h || b.counter !== t || !J) {
                            h && (Ea(a, m), ea(a, m));
        ...
        this.$get = ["$window", "$$jqLite", "$$AnimateRunner", "$timeout", "$$forceReflow", "$sniffer", "$$rAFScheduler", "$animate", function (a, e, y, v, z, l, s, O) {
            ...
            return function (a, c) {
                ...
                var aa = [R, P].join(" ").trim(), ha = k + " " + aa, $ = V(aa, "-active"), k = C.to && 0 < Object.keys(C.to).length;
